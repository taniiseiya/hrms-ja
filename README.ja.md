...
Docker未導入の環境でも見られる**営業デモ用静的HTML**（4業種対応）：

```bash
python3 -m http.server 8080 --directory mock
# → http://localhost:8080/  ← 業種選択ハブ
```

| 業種 | ファイル | 想定単価 |
|---|---|---|
| 🚚 軽貨物・運送業 | `mock/logistics.html` | 初期20万+月3万 |
| 🍴 飲食チェーン | `mock/restaurant.html` | 初期30万+月4万 |
| 🏥 介護事業所 | `mock/caregiving.html` | 初期40万+月5万 |
| ⚖ 士業事務所 | `mock/professional.html` | 初期25万+月3.5万 |
...
- `mock/index.html` — 業種別デモ選択ハブ（顧客に最初に見せる）
- 4業種テンプレ：軽貨物・飲食・介護・士業
- 営業フロー：
  1. 顧客の業種に応じたデモを画面共有
  2. 「お客様の社名・スタッフ名で本番構築します」と提案
  3. 初期費用 + 月額保守 で契約
